		...
		<view class="container">
			<view class="userinfo">
				<view class="userinfo-left">
					<image :src="info.employerHeadPortrait" class="image-avatar"></image>
					<text class="nickname">{{info.employerName}}</text>
				</view>
	...
	export default {
		...
		methods: {
			...
			async accept() {
				...
				this.getOrderDetail()
				return uni.$showMsg("接单成功")
			},
			
			async getOrderDetail() {
				const {
					data: res
				} = await uni.$http.get('/order/getOrder/' + this.id)
				console.log(res)
				if (res.code !== 20000) {
					return uni.$showMsg()
				}
		...
		computed: {
			getTime() {
				return dateTool.getPostTime(this.info.createTime)
